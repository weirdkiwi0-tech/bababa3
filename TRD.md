...
- 삭제 가능 시간 정책: 삭제는 작성일과 무관하게 기한 제한 없이 항상 허용하되, 본인이 작성한 기록(`authorId`가 없거나 현재 `anonymousUserId`와 일치)에 한해서만 허용한다(`canDeleteEntry` 순수 함수로 검증).
- 수정 이력 관리: 일기 수정 발생 시 이전 버전의 `content`와 `updatedAt` 시각을 `history` 배열로 누적 기록하며, 수정 이력이 존재하는 기록에는 "수정됨" 텍스트 표시를 나타내고 클릭 시 수정 시각 및 수정 전 내용 목록을 조회할 수 있도록 한다.
